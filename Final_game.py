...
from GeneticAlgorithm.Train_GA import *
...
def show_start_screen():
    ...
    buttons = [
        ...
        ("Genetic_Train", column1_x, screen_size // 2),
        ("Genetic_Test", column2_x, screen_size // 2),
...
def game_loop_genetic_train():
    ...
    ga = PopulationEvolution()

    if args.inherit:
        ga.inherit_ancestor()
    else:
        ga.randomFirstAncestor()
    ...
    while True:
        ...
        print("generation:", generation, ", record:", record,
              ", best score:", ga.best_individual.score, ", average score of all agents:",
              ga.average_score_of_all_agents)
        ...
        if generation % 25 == 0:
            ga.record_generation()

def game_loop_genetic_test(current_best_score):
    saved_best_genes = os.path.join("GA_data", "saved_genes", "best_record", "reached_" + str(current_best_score))
    with open(saved_best_genes, "r") as f:
        genes = np.array(list(map(float, f.read().split())))

    # 读取相应的seed
    reply_seed = os.path.join("GA_data", "reply_seed", "reached_"+ str(current_best_score))
    with open(reply_seed, "r") as f:
        seed = int(f.read())
...
def game_loop_deep_q_learning_test():
    ...
    snake.append(initial_body_part)
    score = 0
    steps = 0
    food_position = place_food(grid_size, snake)
